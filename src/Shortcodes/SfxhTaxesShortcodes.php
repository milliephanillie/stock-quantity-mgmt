...
abstract class SfxhTaxesShortcodes {
    ...
    public function get_js_resource(string $handle = null) {
        ...
        $resource_file = trailingslashit(SFXH_TAXES_PLUGIN_PATH) . $resource_file_path;

        if(!file_exists($resource_file)) {
            throw new \Exception('Resource file ' . $resource_file . ' does not exist.');
        }

        $resource = trailingslashit(SFXH_TAXES_PLUGIN_URL) . $resource_file_path;
